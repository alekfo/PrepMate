...
def evaluate_answer(question_text: str, answer_text: str, vacancy_context: str) -> dict:
    ...
    text = _ask(f"""Ты — ментор, который готовит кандидата к техническому собеседованию. Оцени его ответ так,
                        как это делал бы поддерживающий наставник, а не строгий экзаменатор: твоя цель — чтобы
                        кандидат видел честную картину, но не терял мотивацию продолжать подготовку.
                        ...
                        Ориентиры для итогового балла (1–10):
                        - 8-10: ответ по существу, есть конкретика или пример — не обязательно "эталонный" учебниковый ответ
                        - 5-7: разумный ответ, в целом раскрывающий тему, пусть без деталей и примеров — нормальный уровень для реального собеседования
                        - 3-4: ответ поверхностный или частично не по теме, но видна попытка разобраться
                        - 1-2: ответ пустой, полностью не по теме или в корне неверный
                        Не занижай балл только за то, что ответ короче или менее полон, чем идеальный — сравнивай с тем,
                        что достаточно для реального прохождения собеседования на эту роль, а не с максимально возможным ответом.
                        При этом strengths и improvements должны оставаться такими же подробными и честными, как обычно —
                        смягчение касается только итогового балла, а не глубины разбора сильных и слабых сторон.
